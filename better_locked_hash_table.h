...
class better_locked_probing_hash_table : public hash_table {
  ...
  private:
    ...
    /****************/
    // const int num_mutex;
    std::mutex mutexes[_NUM_MUTEXES_]; 
    /****************/
    ...
    bool insert(uint32_t key, uint64_t value) {
      ...
      int temp_index = index % _NUM_MUTEXES_;
      mutexes[temp_index].lock();
      ...
      while(table[index].valid == true) {
        if(table[index].key == key) {
          ...
          probe_count++;
          index = this->hash_next(key, index);
          if(temp_index != (index % _NUM_MUTEXES_)){
            mutexes[temp_index].unlock();
            temp_index = (index % _NUM_MUTEXES_);
            mutexes[temp_index].lock();
          }
